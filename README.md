...
# 这是基于bookmarks14插件魔改后的版本😄
# 主要增添的功能：

* 1、分组tag：用户可以在书签前的[]的框里打入tag节点，节点间支持‘-’连接，并可以用 $ 表示它在该节点的序号。比如  [a$1-b-c$2]书签A  这种格式。（使用过的tag会被记录下来，下次打标签时可用tag历史记录快速插入）
* 2、分组搜索：新增搜索区域、书签分组视图。可以通过空格连接、查询同时经历多个节点路径的书签，结果会按tag路径分组展示，组内按各节点$n排序（无序号的默认排在最后，用字典序）
* 3、交叉路径：分组采用了节点为主的设计思想，当你在搜索框输入‘a b’时，tag为[a-b][a-b-c][a-b-c-d]的小组都会被检索到，组内按逐个节点、逐序号排列
* 4、分组整理：新增分组整理区域。因为用户自己维护序号成本高、冲突可能性极大，所以可以在分组整理区域按tag节点搜索书签，可通过拖拽调整它们的顺序，如果插入后发生序号冲突、系统会自动将后面所有书签都按序向后递推，自动完成序号的整理，无需手动维护和校验👍

# ✨目录已包含打包后的visix，将其下载到本地，在vscode扩展商店页面点击右上角‘···’，选择从visix导入即可！
